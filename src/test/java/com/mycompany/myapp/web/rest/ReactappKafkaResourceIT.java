...
class ReactappKafkaResourceIT {
    ...
    void setup() {
        ...
        ReactappKafkaResource kafkaResource = new ReactappKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/reactapp-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/reactapp-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
